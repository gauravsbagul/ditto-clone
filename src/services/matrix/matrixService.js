...
import matrixSdk, { AutoDiscovery, EventTimeline } from 'matrix-js-sdk'
...
const debug = require('debug')('ditto:services:matrix:matrixService')
...
class MatrixService {
  ...
  // Actions
  // ********************************************************************************
  async createClient (baseUrl, userId, accessToken) {
    if (this._client) {
      if (this._client.baseUrl === baseUrl &&
          this._client.getUserId() === userId &&
          this._client.getAccessToken() === accessToken) {
  ...
  getRoomAvatar (matrixRoom) {
    ...
    if (!avatar) {
      if (this.isRoomDirect(matrixRoom.roomId)) avatar = matrixRoom.getAvatarFallbackMember()?.user?.avatarUrl
    }
    return avatar
  }
