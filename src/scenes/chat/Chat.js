...
export default class Chat {
  ...
  _getSnippet () {
    ...
    if (typing.length > 0) {
      ...
      if (typing.length > 1) {
        snippet.content = i18n.t('messages:content.groupTyping', { user1: user.name$.getValue(), others: typing.length - 1 })
      } else {
  ...
  async fetchPreviousMessages () {
    try {
      ...
      await matrix.getClient().paginateEventTimeline(
        this._matrixRoom.getLiveTimeline(),
        { backwards: true }
      )
      ...
      case 'm.image': {
        ...
        if (!response) {
          ...
          return {
            ...
            message: i18n.t('messages:content.contentNotUploadedNotice')
          }
