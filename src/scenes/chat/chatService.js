...
class ChatService {
  ...
  _handleRoomLocalEchoUpdatedEvent (event, matrixRoom, oldEventId) {
    ...
    // If this is an image message, we need to remove the associated pending message
    if (!oldEventId && event.getType() === 'm.room.message' &&
        event.getContent().msgtype === 'm.image' && this._chats.all[roomId]) {
    ...
    if (Message.isMessageUpdate(event)) {
      ...
      if (Message.isMessageUpdate(relatedEvent)) {
        ...
        const mainMessage = messages
          .getMessageByRelationId(relatedEvent.getId(), roomId)
  ...
  _listen () {
    matrix.getClient().on('accountData',
      (event) => this._handleAccountDataEvent(event))
    matrix.getClient().on('deleteRoom',
      (roomId) => this._handleDeleteRoomEvent(roomId))
    matrix.getClient().on('Room.localEchoUpdated',
      (event, room, oldEventId, oldStatus) =>
        this._handleRoomLocalEchoUpdatedEvent(event, room, oldEventId, oldStatus))
    matrix.getClient().on('Room.receipt',
      (event, room) => this._handleRoomReceiptEvent(event, room))
    matrix.getClient().on('Room.timeline',
      (event, room) => this._handleRoomTimelineEvent(event, room))
    matrix.getClient().on('RoomMember.typing',
      (event, member) => this._handleRoomMemberTypingEvent(event, member))
